...
class DaemonCacheTest extends AssertionsForJUnit {
  ...
  @Test def verifyCreateAndGetPools(): Unit = {
    ...
    val pool13 = Await.result(cache.createWalletPool(PoolInfo("pool_3"), "config"), Duration.Inf)
  ...
  @Test def verifyCreateAndDeletePool(): Unit = {
    val poolRandom = Await.result(cache.createWalletPool(PoolInfo("random"), "config"), Duration.Inf)
  ...
  def verifyGetAccountOperations(): Unit = {
    val pool1 = Await.result(DefaultDaemonCache.dbDao.getPoolByName(POOL_NAME), Duration.Inf)
